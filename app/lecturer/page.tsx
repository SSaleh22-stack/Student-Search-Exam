...
export default function LecturerPage() {
  ...
  const handleExportCalendar = () => {
    const icsContent = exams
      .map((exam) => {
        const startDate = new Date(`${exam.examDate}T${exam.periodStart}`);
...
SUMMARY:${exam.courseName} (${exam.courseCode}) - ${exam.section}
DESCRIPTION:${exam.examPeriod} - ${exam.room}${exam.invigilator ? ` - مراقب: ${exam.invigilator}` : ""}
LOCATION:${exam.room}
END:VEVENT`;
      })
      .join("\n");
