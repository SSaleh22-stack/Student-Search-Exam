...
import { parseHijriDate, parseArabicTime, extractDateFromCellText, gregorianToHijri } from "@/lib/utils/hijri-converter";
      ...
      if (fieldName) {
        ...
        if (fieldName === "exam_date") {
          ...
          if (extractedDate) {
            ...
            isEmpty = false;
            value = extractedDate;
            console.log(`[parseExam] Row ${rowNum}: Using cell text value (preserves Hijri/Gregorian as-is): ${extractedDate}`);
          }
          // Fallback to cell.value only if cell.text couldn't be parsed
          else if (value instanceof Date) {
            isEmpty = false;
            const dateObj = value as Date;
            // Excel stores Hijri dates as Gregorian Date objects internally
            // If the format indicates Hijri, convert the Gregorian Date back to Hijri
            if (isHijriFormat) {
              // Convert Gregorian Date back to Hijri (Excel stored it as date format in Hijri)
              ...
              console.log(`[parseExam] Row ${rowNum}: Converted Gregorian Date to Hijri: ${value} (from Gregorian: ${gregorianStr}, numFmt: "${numFmt}")`);
            } else {
              // Keep as Gregorian
              ...
              console.log(`[parseExam] Row ${rowNum}: Date object formatted to: ${value} (Gregorian)`);
            }
          } 
          // Handle Excel serial date number - only if cell text wasn't usable
            ...
            try {
              ...
              // If format indicates Hijri, convert the Gregorian Date back to Hijri
              if (isHijriFormat) {
                // Convert Gregorian Date back to Hijri (Excel stored it as date format in Hijri)
                ...
                console.log(`[parseExam] Row ${rowNum}: Converted Excel serial date to Hijri: ${value} (from Gregorian: ${gregorianStr}, numFmt: "${numFmt}")`);
              } else {
                const year = jsDate.getFullYear();
                const month = String(jsDate.getMonth() + 1).padStart(2, "0");
                const day = String(jsDate.getDate()).padStart(2, "0");
                value = `${year}-${month}-${day}`;
                console.log(`[parseExam] Row ${rowNum}: Excel serial date formatted to: ${value} (Gregorian)`);
              }
            } catch (e) {
