        ...
        if (fieldName === "exam_date") {
          ...
          if (extractedDate) {
            ...
            if (year >= 1200 && year < 1600) {
              ...
              console.log(`[parseExam] Row ${rowNum}: Converted Hijri date ${extractedDate} to Gregorian: ${value}`);
            } else {
              // It's already Gregorian - use as-is
              isEmpty = false;
              value = extractedDate;
              console.log(`[parseExam] Row ${rowNum}: Using cell text value (Gregorian): ${extractedDate}`);
            }
            isEmpty = false;
          }
          // Fallback to cell.value only if cell.text couldn't be parsed
            ...
            console.log(`[parseExam] Row ${rowNum}: Date object formatted to Gregorian: ${value}`);
          } 
          // Handle Excel serial date number - only if cell text wasn't usable
            ...
            try {
              ...
              value = `${year}-${month}-${day}`;
              console.log(`[parseExam] Row ${rowNum}: Excel serial date formatted to Gregorian: ${value}`);
            } catch (e) {
